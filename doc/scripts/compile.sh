...
skipped_headers="$(find $DIR/content -name '*.h' | grep -Ff $SCRIPT_DIR/skip_headers)"

# echo $skipped_headers

if [[ $skipped_headers == *$FILE* ]]; then
    echo "$FILE is currently included in skip_headers"
    exit 1
fi
